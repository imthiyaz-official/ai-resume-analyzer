...
def load_models():
    ...
    try:
        ...
        possible_paths = [
            "clf.pkl",
            "models/clf.pkl",
            "/mount/src/ai-resume-analyzer/clf.pkl"
        ]
        ...
        for path in possible_paths:
            try:
                if os.path.exists(path):
                    ...
                    tfidf = pickle.load(open(path.replace("clf.pkl", "tfidf.pkl"), "rb"))
                    le = pickle.load(open(path.replace("clf.pkl", "encoder.pkl"), "rb"))
...
def create_dummy_models():
    ...
    class DummyTfidf:
        def transform(self, X):
            # Return sparse matrix-like object
...
def extract_text_from_docx(file):
    try:
        ...
        st.warning(f"Error reading DOCX: {str(e)}")
        return ""
...
def extract_text_from_txt(file):
    try:
        ...
        st.warning(f"Error reading TXT: {str(e)}")
        return ""
...
def handle_file_upload(uploaded_file):
    ...
    st.warning("Unsupported file format. Please upload PDF, DOCX, or TXT.")
    return ""
...
def category_confidence(resume_text):
    ...
    try:
        ...
        # Check if we have real models
        if hasattr(tfidf, 'transform') and hasattr(svc_model, 'decision_function'):
            # Transform text
            ...
            if hasattr(svc_model, 'decision_function'):
                try:
                    scores = svc_model.decision_function(vec)
                    if len(scores.shape) > 1:
                        scores = scores[0]
                except:
                    # Fallback to predict_proba
                    if hasattr(svc_model, 'predict_proba'):
                        scores = svc_model.predict_proba(vec)[0]
                    else:
                        scores = [0.5]
            ...
            if len(scores) > 0:
                ...
                if max_score - min_score > 0:
                    confidence = ((max_score - min_score) / (max_score - min_score + 1e-9)) * 100
                else:
                    confidence = 70.0
            else:
                confidence = 70.0
            
            return category, min(confidence, 95)
        else:
            # Fallback to keyword matching
            return fallback_category_detection(resume_text), 70.0
    except Exception as e:
...
def fallback_category_detection(resume_text):
    ...
    role_keywords = {
        "Data Scientist": ["data science", "machine learning", "python", "analysis", "statistics"],
...
def resume_strength_score(text, skills):
    ...
    sections = [
        ("project", 20), ("intern", 20), ("experience", 20), ("certificate", 20),
        ("education", 10), ("certification", 10), ("leadership", 10), ("volunteer", 10),
        ("achievement", 10), ("award", 10), ("github", 10), ("portfolio", 10),
        ("communication", 10), ("teamwork", 10), ("problem solving", 10), ("critical thinking", 10),
        ("adaptability", 10), ("flexibility", 10), ("time management", 10), ("organization", 10),
        ("leadership", 10), ("initiative", 10), ("certified", 10), ("published", 10),
        ("research", 10), ("awards", 10), ("recognition", 10), ("extracurricular", 10),
        ("clubs", 10)
    ]
    
    text_lower = text.lower()
    for keyword, points in sections:
...
def ats_score(resume, jd):
    ...
    try:
        # Try cosine similarity with TF-IDF
...
def resume_suggestions(score, skills):
    tips = []
    if score < 60: 
        tips.append("Add more projects and technical depth.")
    if skills and "Cloud" not in skills: 
        tips.append("Add basic cloud exposure (AWS/Azure/GCP).")
    if skills and "Visualization" not in skills: 
        tips.append("Mention data visualization tools like Power BI or Tableau.")
    if not tips: 
        tips.append("Resume looks strong. Tailor it to job roles.")
    return tips
                ...
                if st.button("Generate Resume",
                    ...
                    if output_format == "PDF":
                        ...
                        try:
                            ...
                            st.success("PDF generated successfully! Click download button above.")
                        except Exception as e:
...
def render_analysis_tab():
    ...
    with col1:
        ...
        resume_text = st.text_area(
            ...
            value=st.session_state.get('current_resume_text', '')
        )
    ...
    if resume_text:
        ...
        conf_display = conf if conf else 50
        
        # Use sample data if confidence is low
        if conf_display < 30:
            sample_data = True
            conf_display = 70  # Show higher confidence for sample
        else:
            sample_data = False
        ...
        <p><b>{conf_display}% Confidence</b> {sample_data and '<span style="color: #6366f1; font-size: 0.8em;">(estimated)</span>' or ''}</p>
        <style>
        @keyframes progressFill {{
            from {{ width: 0%; }}
            to {{ width: {conf_display}%; }}
        }}
        </style>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            if category:
                ...
                st.markdown('<div class="empty-state-icon">📄</div>')
                st.markdown('<p>No category</p>')
                st.markdown('</div>')
            st.markdown('</div>', unsafe_allow_html=True)
        with col2:
            st.markdown('<div class="card celebration">', unsafe_allow_html=True)
        ...
        with anal_tab2:
            ...
            with col1:
                if skills:
                    ...
                    if fig:
                        ...
                        st.info("Unable to create pie chart")
                else:
                    st.info("No skills detected for visualization")
            with col2:
                if skills:
                    fig = radar_chart(skills)
                    if fig:
                        st.pyplot(fig)
                    else:
                        st.info("Unable to create radar chart")
                else:
            ...
            if st.button("Create Visual Resume Image", key="generate_visual_resume"):
                try:
                    ...
                    st.error(f"Error generating visual resume: {str(e)}")
